...
const Auction = ({handleBid,handleEnable}) => {
    ...
    return (
        ...
        {grid===true? <div className='flex items-center gap-5'><TfiLayoutGrid2 size={30} /><p className='text-2xl font-bold'>Grid View</p></div>: <div className='flex items-center gap-5'> <GiHamburgerMenu size={30} /><p className='text-2xl font-bold'>List View</p></div>}</button>
            ...
            <div className={`grid sm:grid-cols-2 ${grid===true? "visible":"hidden"}  grid-cols-1 gap-5 `}>
                {items.map(auctionItems => <Suspense fallback={<h3>data loading</h3>}><AuctionItems handleEnable={handleEnable} key={auctionItems.id} auctionItems={auctionItems} handleBid={handleBid} ></AuctionItems></Suspense>)}
            </div>


            <div className={` ${grid===true? "hidden" :"visible"}   `}>
                <AuctionItemsTable items={items} handleEnable={handleEnable} handleBid={handleBid} ></AuctionItemsTable>
            </div>
        </div>
    );
};
